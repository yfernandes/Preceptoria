...
export class PermissionsGuard implements CanActivate {
	...
	async canActivate(context: ExecutionContext): Promise<boolean> {
		const resourceServiceType = this.reflector.get<any>(PERMISSIONS_KEY, context.getHandler())
		...
		const resourceService: IsAllowedAccess = this.serviceRegistry.getService(resourceServiceType)
