...
test.describe("Classes Data", () => {
	test("should fetch classes from API with authentication", async ({ request }) => {
		const loginResponse = await request.post("http://localhost:3000/auth/signin", {
			data: {
	...
	test("should display classes data or empty state on classes page after login", async ({
		...
		const tableVisible = await page.locator('[data-testid="classes-table"]').isVisible()
		const emptyVisible = await page.locator("text=No classes found").isVisible()

		if (!tableVisible && !emptyVisible) {
			const content = await page.content()
			throw new Error(`Neither classes table nor empty state found. Page content: ${content}`)
		}
	})
