...
const defaultConfig: Required<Omit<JwtConfig, "issuer" | "audience" | "clockTolerance">> & {
	issuer?: string
	audience?: string
	clockTolerance: string
} = {
	...
	return {
		...
		async sign(
			...
			try {
				...
				const expiry =
					options?.expiry ??
					(tokenType === "access" ? finalConfig.accessTokenExpiry : finalConfig.refreshTokenExpiry)
		...
		async verify(token: string): Promise<CustomJwtPayload | false> {
			...
			try {
				...
				if (
					!payload.roles.every((r: unknown) => Object.values(UserRoles).includes(r as UserRoles))
				) {
					return false
				}
