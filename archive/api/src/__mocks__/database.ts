...
export class MockDatabase {
	...
	findDocumentsByStudent(studentId: string): MockDocument[] {
		return Array.from(this.documents.values()).filter((doc) => doc.student.id === studentId)
	}

	createDocument(documentData: Omit<MockDocument, "id" | "createdAt" | "updatedAt">): MockDocument {
	...
	findShiftsByHospital(hospitalId: string): MockShift[] {
		return Array.from(this.shifts.values()).filter((shift) => shift.hospital.id === hospitalId)
	}

	findShiftsByPreceptor(preceptorId: string): MockShift[] {
		return Array.from(this.shifts.values()).filter((shift) => shift.preceptor.id === preceptorId)
	}
	...
	findClassesByCourse(courseId: string): MockClass[] {
		return Array.from(this.classes.values()).filter((cls) => cls.course.id === courseId)
	}
...
export const createDatabaseMock = () => {
	const mockUserRepository = {
		...
		create: mock((userData: Omit<MockUser, "id">) => Promise.resolve(mockDb.createUser(userData))),
	...
	const mockDocumentRepository = {
		...
		create: mock((documentData: Omit<MockDocument, "id" | "createdAt" | "updatedAt">) =>
	...
	const mockShiftRepository = {
		...
		find: mock((query: { hospital?: { id?: string }; preceptor?: { id?: string } }) => {
