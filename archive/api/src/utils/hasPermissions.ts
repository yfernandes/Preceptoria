...
import { type Actions, type Modifiers, type Resource, rolesPermissions } from "./permissions"
...
export async function hasPermission(
	...
	for (const role of requester.roles) {
		...
		if (permissions.includes(`${resource}:${action}_Students`)) {
			const checkStudentsAccess = resolvers[role as UserRoles][resource]?.Students
...
type OwnershipResolver = (requester: UserContext, resourceId: string) => Promise<boolean> | boolean
		...
		Student: {
			Own: async (requester, resourceId) => {
				try {
					const student = await db.student.findOne({ id: resourceId })
					return (
						student?.shifts.exists((shift) => shift.preceptor.id === requester.preceptorId) ?? false
					)
				} catch (error) {
		...
		Hospital: {
			Own: async (requester, resourceId) => {
				try {
					...
					return (
						hospital?.shifts.exists((shift) => shift.preceptor.id === requester.preceptorId) ??
						false
					)
				} catch (error) {
