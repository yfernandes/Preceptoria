...
import { Classes, Course, Document, School, Student, Supervisor, User } from "../modules/entities"
import { GoogleSheetsService, type GoogleSheetsSubmission } from "./googleSheets"
...
export class SyncService {
	...
	async syncFromGoogleSheets(spreadsheetId: string, range = "JoinedForm"): Promise<SyncResult> {
		...
		try {
			...
			console.log(`Sync completed. Processed ${submissions.length.toString()} submissions.`)
			console.log(
	...
	private async processDocuments(
		...
		for (const url of documentation.badgePicture) {
			await this.createDocumentFromUrl(url, student, DocumentType.BADGE_PICTURE, result)
		}
	}
