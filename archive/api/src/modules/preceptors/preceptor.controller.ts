...
export const preceptorController = new Elysia({ prefix: "/preceptors" })
	...
	.post(
		"/",
		async ({ body: { userId, hospitalId, specialty, licenseNumber }, requester }) => {
			try {
				// Check permissions for creating preceptors
				const hasAccess = await hasPermission(requester, Resource.Preceptor, Actions.Create, "")
				...
				const hasUserAccess = await hasPermission(requester, Resource.User, Actions.Read, userId)

				if (!hasUserAccess) {
					return error(403, {
						success: false,
						message: "You don't have permission to assign this user as a preceptor",
					})
				}
				...
				if (!hasHospitalAccess) {
					return error(403, {
						success: false,
						message: "You don't have permission to assign preceptors to this hospital",
					})
				}
		...
		try {
			...
			return {
				...
				pagination: {
					...
					limit: parseInt(limit as string, 10),
					offset: parseInt(offset as string, 10),
					hasMore: accessiblePreceptors.length === parseInt(limit as string, 10),
				},
			}
		} catch (err) {
	...
	.get("/:id", async ({ params: { id }, requester }) => {
		try {
			...
			const hasAccess = await hasPermission(requester, Resource.Preceptor, Actions.Read, id)
	...
	.patch(
		...
		async ({ params: { id }, body, requester }) => {
			try {
				...
				const hasAccess = await hasPermission(requester, Resource.Preceptor, Actions.Update, id)
	...
	.delete("/:id", async ({ params: { id }, requester }) => {
		try {
			...
			const hasAccess = await hasPermission(requester, Resource.Preceptor, Actions.Delete, id)
			...
			const preceptor = await db.preceptor.findOne({ id }, { populate: ["shifts"] })
