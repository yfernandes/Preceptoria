...
export class OrgAdminService {
	async createOrgAdmin(
		...
		const hasAccess = await hasPermission(requester, Resource.OrgAdmin, Actions.Create, "")
		if (!hasAccess)
			return {
				status: 403,
		...
		if (existingOrgAdmin) return { status: 400, error: "User is already an org admin" }
	...
	async getOrgAdmins(
		...
		for (const orgAdmin of orgAdmins) {
			const hasAccess = await hasPermission(requester, Resource.OrgAdmin, Actions.Read, orgAdmin.id)
		...
		return {
			...
			pagination: {
				...
				offset: typeof offset === "string" ? parseInt(offset, 10) : offset,
				hasMore:
					accessibleOrgAdmins.length === (typeof limit === "string" ? parseInt(limit, 10) : limit),
			},
		}
	}

	async getOrgAdminById(requester: UserContext, id: string) {
		const hasAccess = await hasPermission(requester, Resource.OrgAdmin, Actions.Read, id)
		if (!hasAccess)
			return {
				status: 403,
				error: "You don't have permission to view this org admin",
			}
		const orgAdmin = await db.orgAdmin.findOne({ id }, { populate: ["user", "hospital", "school"] })
	...
	async updateOrgAdmin(
		...
		const hasAccess = await hasPermission(requester, Resource.OrgAdmin, Actions.Update, id)
		if (!hasAccess)
			return {
				status: 403,
	...
	async deleteOrgAdmin(requester: UserContext, id: string) {
		const hasAccess = await hasPermission(requester, Resource.OrgAdmin, Actions.Delete, id)
		if (!hasAccess)
			return {
				status: 403,
