...
export class ClassesService {
	async createClass(requester: UserContext, body: { name: string; courseId: string }) {
		const hasAccess = await hasPermission(requester, Resource.Classes, Actions.Create, "")
		if (!hasAccess) {
			return {
				status: 403,
				success: false,
	...
	async getAllClasses(
		...
		for (const classItem of classes) {
			const hasAccess = await hasPermission(requester, Resource.Classes, Actions.Read, classItem.id)
			if (hasAccess) {
		...
		return {
			...
			pagination: {
				...
				limit: parseInt(limit as string, 10),
				offset: parseInt(offset as string, 10),
				hasMore: accessibleClasses.length === parseInt(limit as string, 10),
			},
		}
	}

	async getClassById(requester: UserContext, id: string) {
		const hasAccess = await hasPermission(requester, Resource.Classes, Actions.Read, id)
		if (!hasAccess) {
			return {
				status: 403,
				success: false,
	...
	async updateClass(
		...
		const hasAccess = await hasPermission(requester, Resource.Classes, Actions.Update, id)
		if (!hasAccess) {
			return {
				status: 403,
				success: false,
		...
		if (body.courseId) {
			...
			if (!hasCourseAccess) {
				return {
					status: 403,
					success: false,
					message: "You don't have permission to assign this class to the specified course",
				}
			}
	...
	async deleteClass(requester: UserContext, id: string) {
		const hasAccess = await hasPermission(requester, Resource.Classes, Actions.Delete, id)
		if (!hasAccess) {
			return {
				status: 403,
				success: false,
		...
		const classItem = await db.classes.findOne({ id }, { populate: ["students"] })
		if (!classItem) {
			return {
				status: 404,
				success: false,
		...
		if (classItem.students.length > 0) {
			return {
				status: 400,
				success: false,
				message: "Cannot delete class that has students. Please reassign or remove students first.",
			}
		}
