...
describe("Auth Integration Tests", () => {
	...
	beforeAll(async () => {
		...
		serverProcess = Bun.spawn(["bun", "run", "src/index.ts", `--port=${TEST_PORT.toString()}`], {
		...
		if (attempts >= maxAttempts) {
			throw new Error(`Test server failed to start on port ${TEST_PORT.toString()}`)
		}
	})
	...
	describe("Authentication Flow", () => {
		...
		it("should be able to access a protected route, with the default SysAdmin User", async () => {
			...
			const loginResponse = await testClient.post<LoginResponse>("/auth/signin", {
				email: defaultSysAdminCredentials.email,
				password: defaultSysAdminCredentials.password,
			})
			...
			const protectedResponse = await authenticatedClient.get<ClassesResponse>("/classes")
