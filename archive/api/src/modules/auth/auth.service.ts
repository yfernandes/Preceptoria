 ...
 */
export const handleValidationError = (err: unknown): ValidationErrorResponse | null => {
	if (Array.isArray(err) && err.length > 0 && err.every((e) => e instanceof ValidationError)) {
		return {
			success: false,
...
export const handleSignUp = async (input: AuthInput): Promise<AuthResult> => {
	try {
		...
		const user = await User.create(input.name, input.email, input.phone, input.password)
...
export const handleSignIn = async (input: SignInInput): Promise<AuthResult> => {
	try {
		const user = await db.user.findOne({ email: input.email }, { populate: ["passwordHash"] })
 ...
 */
export const handleTokenRefresh = async (refreshToken: string): Promise<AuthResult> => {
	if (!refreshToken) {
		return {
			success: false,
