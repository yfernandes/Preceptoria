...
export const userController = new Elysia({ prefix: "/users" })
	...
	.post(
		...
		async ({ body: { name, email, phone, password }, requester }) => {
			try {
				...
				const hasAccess = await hasPermission(requester, Resource.User, Actions.Create, "")
	...
	.get("/", async ({ requester, query }) => {
		try {
			...
			const hasAccess = await hasPermission(requester, Resource.User, Actions.Read, "")
			...
			return {
				...
				pagination: {
					...
					limit: parseInt(limit as string, 10),
					offset: parseInt(offset as string, 10),
					hasMore: users.length === parseInt(limit as string, 10),
				},
			}
		} catch (err) {
	...
	.get("/:id", async ({ params: { id }, requester }) => {
		try {
			...
			const hasAccess = await hasPermission(requester, Resource.User, Actions.Read, id)
	...
	.patch(
		...
		async ({ params: { id }, body, requester }) => {
			try {
				...
				const hasAccess = await hasPermission(requester, Resource.User, Actions.Update, id)
	...
	.delete("/:id", async ({ params: { id }, requester }) => {
		try {
			...
			const hasAccess = await hasPermission(requester, Resource.User, Actions.Delete, id)
