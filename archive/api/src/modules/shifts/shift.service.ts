...
export class ShiftService {
	async createShift(
		...
		const hasAccess = await hasPermission(requester, Resource.Shift, Actions.Create, "")
		if (!hasAccess) {
			return {
				status: 403,
				success: false,
	...
	async getAllShifts(
		...
		for (const shift of shifts) {
			const hasAccess = await hasPermission(requester, Resource.Shift, Actions.Read, shift.id)
		...
		return {
			...
			pagination: {
				...
				limit: parseInt(limit as string, 10),
				offset: parseInt(offset as string, 10),
				hasMore: accessibleShifts.length === parseInt(limit as string, 10),
			},
		}
	}
	...
	async getShiftById(requester: UserContext, id: string) {
		...
		const hasAccess = await hasPermission(requester, Resource.Shift, Actions.Read, id)
		if (!hasAccess) {
			return {
				status: 403,
				success: false,
		...
		const shift = await db.shift.findOne(
			{ id },
			{
				populate: ["hospital", "preceptor", "students", "students.class", "students.class.course"],
			}
		)
		if (!shift) {
			return {
	...
	async updateShift(
		...
		const hasAccess = await hasPermission(requester, Resource.Shift, Actions.Update, id)
		if (!hasAccess) {
			return {
				status: 403,
				success: false,
	...
	async deleteShift(requester: UserContext, id: string) {
		...
		const hasAccess = await hasPermission(requester, Resource.Shift, Actions.Delete, id)
		if (!hasAccess) {
			return {
				status: 403,
				success: false,
