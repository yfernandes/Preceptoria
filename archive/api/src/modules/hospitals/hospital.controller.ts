...
export const hospitalController = new Elysia({ prefix: "/hospitals" })
	...
	.get("/", async ({ requester, status }) => {
		try {
			...
			if (requester.orgAdminId) {
				...
				hospitals = await db.hospital.find({}, { populate: ["manager.user", "shifts"] })
	...
	.patch(
		...
		async ({ params: { id }, body, requester, status }) => {
			try {
				...
				// Find hospital
				const hospital = await db.hospital.findOne({ id }, { populate: ["manager.user"] })
	...
	.delete("/:id", async ({ params: { id }, requester, status }) => {
		try {
			...
			// Find hospital
			const hospital = await db.hospital.findOne({ id }, { populate: ["manager.user", "shifts"] })
