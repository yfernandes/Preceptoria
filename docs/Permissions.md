...
const studentsController = new Elysia({ prefix: "/students" })
	...
	.get("/:id", async ({ params, user }) => {
		...
		const hasAccess = await hasPermission(user, Resource.Student, Actions.Read, params.id);
...
test("supervisor can read their own students", async () => {
	...
	const hasAccess = await hasPermission(supervisor, Resource.Student, Actions.Read, student.id);
...
test("student cannot see students from other classes", async () => {
	...
	const hasAccess = await hasPermission(student, Resource.Student, Actions.Read, otherStudent.id);
