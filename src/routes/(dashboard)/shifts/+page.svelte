				...
				>
					<div class="grid grid-cols-1 gap-6">
						<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
							...
							<div class="space-y-1.5">
								<label class="text-sm font-medium text-gray-700" for="preceptorId"
									>Preceptor Responsável</label
								>
								<select
									name="preceptorId"
									id="preceptorId"
						...
						<Input
							label="Local Específico (Setor/Ala)"
							name="location"
			...
			<Card class="flex flex-col gap-6 p-6">
				...
				<div class="space-y-3">
					<div class="flex items-center justify-between border-t border-gray-50 pt-4">
						<h4 class="text-xs font-bold tracking-widest text-gray-400 uppercase">
							Estudantes Escalados
						</h4>
						<Badge variant="neutral" class="border-none bg-gray-100 text-gray-600">
					...
					<div class="flex flex-wrap gap-2">
						{#each shift.students as entry}
							<div
								class="flex items-center gap-2 rounded-full bg-gray-50 py-1 pr-2 pl-1 text-xs font-medium text-gray-700 transition-colors hover:bg-gray-100"
							>
								<div
						...
						<Button
							variant="ghost"
							size="sm"
							class="h-8 rounded-full border-dashed border-gray-200 text-[10px]"
				...
				{#if selectedShiftId === shift.id}
					<div
						class="animate-in slide-in-from-top-2 mt-4 rounded-2xl bg-blue-50/50 p-4 ring-1 ring-blue-100"
					>
						<div class="mb-3 flex items-center justify-between">
							<span class="text-xs font-bold text-blue-700 uppercase">Atribuir Estudante</span>
							<button onclick={() => (selectedShiftId = null)}
								><X class="h-4 w-4 text-blue-400" /></button
							>
						</div>
						<form
							method="POST"
						...
						>
							...
							<select
								name="studentId"
								required
								class="flex-1 rounded-lg border border-blue-200 bg-white px-3 py-2 text-sm shadow-sm outline-none"
							>
								<option value="">Selecione um estudante...</option>
								{#each data.students as s}
									{@const allDocsApproved =
										s.documents.length > 0 && s.documents.every((d) => d.status === "APPROVED")}
									<option value={s.id} disabled={!allDocsApproved}>
										{s.user.name}
										{!allDocsApproved ? "(Docs Pendentes)" : ""}
									</option>
								{/each}
							</select>
