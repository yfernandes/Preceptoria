				...
				>
					<div class="grid grid-cols-1 gap-6">
						...
						<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
							...
							<div class="space-y-1.5">
								<label class="text-sm font-medium text-gray-700" for="supervisorId">Supervisor</label>
								<select
									name="supervisorId"
									id="supervisorId"
								...
								>
									...
									{#each data.supervisors as sup}
										<option value={sup.id} selected={data.user?.role === 'Supervisor' && sup.userId === data.user?.id}>
											{sup.user.name}
										</option>
									{/each}
								</select>
					...
					<table class="min-w-full divide-y divide-gray-100">
						<thead>
							<tr class="bg-gray-50/50">
								<th class="px-6 py-4 text-left text-xs font-bold tracking-wider text-gray-500 uppercase">Turma</th>
								<th class="px-6 py-4 text-left text-xs font-bold tracking-wider text-gray-500 uppercase">Status</th>
								<th class="px-6 py-4 text-left text-xs font-bold tracking-wider text-gray-500 uppercase">Curso / Escola</th>
								<th class="px-6 py-4 text-left text-xs font-bold tracking-wider text-gray-500 uppercase">Supervisor</th>
								<th class="px-6 py-4 text-left text-xs font-bold tracking-wider text-gray-500 uppercase">Período</th>
								<th class="px-6 py-4 text-right text-xs font-bold tracking-wider text-gray-500 uppercase">Ações</th>
							</tr>
						</thead>
								...
								<tr class="group transition-colors hover:bg-gray-50/80">
									...
									<td class="px-6 py-4 whitespace-nowrap">
										{#if classItem.status === 'ACTIVE'}
											<Badge variant="info" class="bg-emerald-50 text-emerald-700 border-emerald-100">Ativa</Badge>
										{:else}
											<Badge variant="neutral" class="bg-gray-50 text-gray-500 border-gray-200">Finalizada</Badge>
										{/if}
									</td>
									<td class="px-6 py-4 whitespace-nowrap">
									...
									<td class="px-6 py-4 whitespace-nowrap">
										<div class="flex items-center gap-2">
											...
											<span class="text-sm text-gray-600">{classItem.supervisor?.user.name || 'Não atribuído'}</span>
										</div>
									</td>
									<td class="px-6 py-4 whitespace-nowrap">
										...
										{#if classItem.status === 'ACTIVE'}
											<form method="POST" action="?/complete" use:enhance>
												...
												<Button 
													...
													variant="outline" 
													size="sm" 
													class="text-amber-600 border-amber-100 hover:bg-amber-50"
												>
