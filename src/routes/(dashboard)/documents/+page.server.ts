...
import { getPresignedDownloadUrl, getPresignedUploadUrl } from "$lib/server/r2"
...
export const actions: Actions = {
	getUploadUrl: async ({ request, locals }) => {
		...
		const documentType = formData.get("documentType")?.toString() as unknown
		const studentId = formData.get("studentId")?.toString()
		const fileSize = parseInt(formData.get("size")?.toString() || "0", 10)
