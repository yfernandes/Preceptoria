...
<div class="animate-in fade-in space-y-8 duration-500">
	<div class="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
		<div>
			...
			<p class="mt-1 text-gray-500">
				Gerencie os hospitais parceiros e seus respectivos gestores.
			</p>
		</div>
		<div class="flex gap-3">
			<Button
				onclick={toggleInvite}
				variant={isInviting ? 'outline' : 'secondary'}
			>
				{#if isInviting}
			...
			<Card title="Cadastrar Novo Hospital">
				...
				>
					<div class="grid grid-cols-1 gap-6">
						<Input label="Nome do Hospital" name="name" placeholder="Ex: Hospital Central" required />
						<Input label="Endereço" name="address" placeholder="Rua, Número, Bairro, Cidade" />
						<div class="space-y-1.5">
							<label class="text-sm font-medium text-gray-700" for="organizationId">Organização</label>
							<select
								name="organizationId"
								id="organizationId"
	...
	{:else if isInviting}
		<div class="mx-auto w-full max-w-2xl">
			<Card title="Convidar Gestor Hospitalar" description="O gestor terá acesso administrativo aos dados de estudantes no hospital selecionado.">
				<form
					method="POST"
				...
				>
					<div class="grid grid-cols-1 gap-6">
						<Input label="E-mail do Gestor" name="email" type="email" placeholder="gestor@hospital.com" required />
						<div class="space-y-1.5">
							<label class="text-sm font-medium text-gray-700" for="hospitalId">Hospital Responsável</label>
							<select
								name="hospitalId"
								id="hospitalId"
								required
						...
						<table class="min-w-full divide-y divide-gray-100">
							<thead>
								<tr class="bg-gray-50/50">
									<th class="px-6 py-4 text-left text-xs font-bold tracking-wider text-gray-500 uppercase">Gestor Convidado</th>
