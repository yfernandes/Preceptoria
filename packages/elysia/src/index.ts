...
console.log(`✅ Environment variables validated successfully (${Bun.env.NODE_ENV || 'development'} mode)`);
...
try {
  const app = new Elysia()
    ...
    .on("afterHandle", ({ request, set }) => {
      ...
      console.log(`${statusEmoji} ${new Date().toISOString()} - ${request.method} ${request.url} - ${statusCode}`);
    })
    ...
    .on("error", ({ error, set }) => {
      ...
      if (error instanceof Error) {
        if (error.message.includes('duplicate key') || error.message.includes('unique constraint')) {
          set.status = 409;
          return { success: false, message: "Resource already exists", error: "CONFLICT" };
        }
        if (error.message.includes('validation') || error.message.includes('ValidationError')) {
          set.status = 400;
          return { success: false, message: "Validation failed", error: "VALIDATION_ERROR" };
        }
        if (error.message.includes('Authentication failed') || error.message.includes('Unauthorized')) {
          set.status = 401;
          return { success: false, message: "Authentication failed", error: "UNAUTHORIZED" };
        }
        ...
        if (error.message.includes('not found') || error.message.includes('Not Found')) {
          set.status = 404;
          return { success: false, message: "Resource not found", error: "NOT_FOUND" };
        }
      }
      set.status = 500;
