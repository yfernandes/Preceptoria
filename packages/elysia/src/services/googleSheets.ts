...
export class GoogleSheetsService {
	...
	async getSubmissions(spreadsheetId: string, range: string = "JoinedForm"): Promise<GoogleSheetsSubmission[]> {
		try {
			...
			const submissions = raw.data.values.slice(1).map((entry): GoogleSheetsSubmission => {
				return {
			...
			console.log(`Fetched ${submissions.length} submissions from Google Sheets`);
			return submissions;
		} catch (error) {
