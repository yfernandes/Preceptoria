...
import { Hospital, HospitalManager } from "../entities";
import { db } from "../db";
import { authMiddleware } from "../middlewares/auth";
import { hasPermission } from "../utils/hasPermissions";
import { Actions, Resource, Modifiers } from "../utils/permissions";
...
export const hospitalController = new Elysia({ prefix: "/hospitals" })
	...
	.post(
		"/",
		async ({ body: { name, address, managerId, phone, email }, requester, status }) => {
			try {
						// Check permissions
		if (!(await hasPermission(requester, Resource.Hospital, Actions.Create, ""))) {
			return status(403, { error: "Insufficient permissions to create hospitals" });
		}
				...
				if (manager.hospital && manager.hospital.id) {
					return status(400, { error: "Hospital manager is already assigned to another hospital" });
				}

				// Check for duplicate hospital name within the organization
				const existingHospital = await db.hospital.findOne({ name });
				if (existingHospital) {
					return status(400, { error: "Hospital with this name already exists" });
				}
	...
	.get("/", async ({ requester, status }) => {
		try {
			// Check permissions
			if (!(await hasPermission(requester, Resource.Hospital, Actions.Read, ""))) {
				return status(403, { error: "Insufficient permissions to read hospitals" });
			}
			...
			} else if (requester.supervisorId || requester.preceptorId || requester.studentId) {
	...
	.get("/:id", async ({ params: { id }, requester, status }) => {
		try {
			// Check permissions
			if (!(await hasPermission(requester, Resource.Hospital, Actions.Read, id))) {
				return status(403, { error: "Insufficient permissions to read this hospital" });
			}
	...
	.patch(
		...
		async ({ params: { id }, body, requester, status }) => {
			try {
							// Check permissions
			if (!(await hasPermission(requester, Resource.Hospital, Actions.Update, id))) {
				return status(403, { error: "Insufficient permissions to update this hospital" });
			}
			...
			if (body.managerId) {
				const currentManager = await db.hospitalManager.findOne({ hospital: id });
				...
				if (body.name && body.name !== hospital.name) {
					const existingHospital = await db.hospital.findOne({ name: body.name });
					if (existingHospital) {
						return status(400, { error: "Hospital with this name already exists" });
					}
				}
	...
	.delete("/:id", async ({ params: { id }, requester, status }) => {
		try {
			// Check permissions
			if (!(await hasPermission(requester, Resource.Hospital, Actions.Delete, id))) {
				return status(403, { error: "Insufficient permissions to delete this hospital" });
			}
			...
			if (hospital.shifts && hospital.shifts.length > 0) {
				return status(400, { 
					error: "Cannot delete hospital with active shifts. Please reassign or cancel all shifts first." 
				});
			}
