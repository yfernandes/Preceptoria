...
export class Migration20250224223635 extends Migration {
	override up(): void {
		this.addSql(
		...
		this.addSql(
			`create table "user" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "name" varchar(255) not null, "email" varchar(255) not null, "phone_number" varchar(255) not null, "password_hash" varchar(255) not null, "roles" text[] not null default '{}', constraint "user_pkey" primary key ("id"));`
		);
		...
		this.addSql(
			`create table "document" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "student_id" uuid not null, "name" varchar(255) not null, "type" text check ("type" in ('')) not null, "url" varchar(255) not null, "uploaded_at" timestamptz not null, "verified" boolean not null default false, constraint "document_pkey" primary key ("id"));`
		);
		...
		this.addSql(
			`create table "student_shifts" ("student_id" uuid not null, "shift_id" uuid not null, constraint "student_shifts_pkey" primary key ("student_id", "shift_id"));`
		);
		...
		this.addSql(
			`alter table "document" add constraint "document_student_id_foreign" foreign key ("student_id") references "student" ("id") on update cascade;`
		);
		...
		this.addSql(
			`alter table "student_shifts" add constraint "student_shifts_student_id_foreign" foreign key ("student_id") references "student" ("id") on update cascade on delete cascade;`
		);
		this.addSql(
			`alter table "student_shifts" add constraint "student_shifts_shift_id_foreign" foreign key ("shift_id") references "shift" ("id") on update cascade on delete cascade;`
		);
