...
export default function DocumentsPage() {
	...
	const updateValidationChecks = async () => {
		...
		try {
			const response = await fetch(
				`/api/documents/${selectedDocument.id}/validation`,
				{
					method: "PATCH",
	...
	const approveDocument = async () => {
		...
		try {
			const response = await fetch(
				`/api/documents/${selectedDocument.id}/approve`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
	...
	const rejectDocument = async () => {
		...
		try {
			const response = await fetch(
				`/api/documents/${selectedDocument.id}/reject`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
					...
					{selectedDocument ? (
						<Card>
							...
							<CardContent className="space-y-6">
								...
								<div className="grid grid-cols-2 gap-4 text-sm">
									<div>
										...
										<p className="text-gray-500">
											CREFITO: {selectedDocument.student.crefito}
										</p>
									</div>
									<div>
								...
								{/* Document Display */}
								<div>
									<h3 className="mb-2 font-medium">Visualizar Documento</h3>
									<div className="rounded-lg border bg-gray-50 p-4">
										...
										<div className="flex min-h-64 items-center justify-center rounded border bg-white p-4">
								...
								{selectedDocument.validationTemplate && (
									<div>
										<h3 className="mb-2 font-medium">Critérios de Validação</h3>
										...
										<div className="space-y-2">
											{selectedDocument.validationTemplate.checks.map(
												(check) => (
													<div
														key={check.id}
								...
								{selectedDocument.status === "PENDING" && (
									<div>
										<h3 className="mb-2 font-medium">Motivo da Rejeição</h3>
										<Textarea
