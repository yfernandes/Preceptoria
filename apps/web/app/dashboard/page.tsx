...
export default function SupervisorDashboard() {
	...
	const { classes, stats, loading, error, getShiftsByDate, getUpcomingShifts } =
		useDashboardData();

	// Memoized computed values
	const todayShifts = useMemo(
		() => getShiftsByDate(selectedDate),
		[selectedDate, getShiftsByDate]
	);
	const upcomingShifts = useMemo(
		() => getUpcomingShifts(selectedDate),
		[selectedDate, getUpcomingShifts]
	);
	...
	if (loading) {
		return (
			<DashboardLayout
				...
				subtitle={DASHBOARD_CONSTANTS.LOADING_TEXT}
				notificationCount={DASHBOARD_CONSTANTS.NOTIFICATION_COUNT}
			>
				<div className="flex flex-1 items-center justify-center">
					<LoadingSpinner
						size="lg"
						text={DASHBOARD_CONSTANTS.LOADING_DATA_TEXT}
					/>
				</div>
			</DashboardLayout>
		);
	...
	if (error) {
		return (
			<DashboardLayout
				title={DASHBOARD_CONSTANTS.TITLE}
				subtitle={DASHBOARD_CONSTANTS.ERROR_SUBTITLE}
				notificationCount={DASHBOARD_CONSTANTS.NOTIFICATION_COUNT}
			>
				<div className="flex flex-1 items-center justify-center">
	...
	return (
		<ErrorBoundary>
			<DashboardLayout
				title={DASHBOARD_CONSTANTS.TITLE}
				subtitle={DASHBOARD_CONSTANTS.SUBTITLE}
				notificationCount={DASHBOARD_CONSTANTS.NOTIFICATION_COUNT}
			>
...
function ClassProgressSection({
	classes,
	onViewDetails,
}: ClassProgressSectionProps) {
	return (
		<Card>
			<CardHeader>
