...
describe('Hospitals Service', () => {
	...
	it('should create a hospital', async () => {
		const mockHospital = { id: '1', name: 'Test Hospital', address: 'Test St', organizationId: 'org1' };
		(db.insert({} as any).values({} as any).returning as any).mockResolvedValue([mockHospital]);

		const result = await hospitalService.createHospital({ name: 'Test Hospital', address: 'Test St', organizationId: 'org1' });
