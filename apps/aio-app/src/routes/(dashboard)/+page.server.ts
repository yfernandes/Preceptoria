...
import { documents, students, hospitals, internshipPlacements } from '$lib/server/db/schema';
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const [pendingDocsCount] = await db.select({ value: count() }).from(documents).where(eq(documents.status, 'PENDING'));
	...
	const [activePlacements] = await db.select({ value: count() }).from(internshipPlacements).where(eq(internshipPlacements.status, 'ACTIVE'));
